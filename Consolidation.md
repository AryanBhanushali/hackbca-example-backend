...
2. **Then add one section at a time, in the fixed section order below**, each as a
   separate edit/append. Finish and flush each section before generating the next.
   Never hold a whole document in the buffer.
...
Do these steps in order. Each is a checkpoint — complete and flush it before the
next.
